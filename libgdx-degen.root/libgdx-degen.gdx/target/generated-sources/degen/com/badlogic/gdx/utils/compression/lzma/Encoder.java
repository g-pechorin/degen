...
public class Encoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init() { com.badlogic.gdx.utils.compression.rangecoder.Encoder.InitBitModels(m_Encoders); }
	...
	int GetRepLen1Price(int state, int posState)
	{
		return com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(_isRepG0[state]) +
				com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice +
				com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price +
						com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice =
								...
								com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
