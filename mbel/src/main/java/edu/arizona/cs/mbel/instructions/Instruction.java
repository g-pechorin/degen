...
public abstract class Instruction{
   ...
   */
   ...
   /** The Instruction factory method for parsing instructions from a buffer. 
     ...
     */
   public static Instruction readInstruction(edu.arizona.cs.mbel.mbel.ClassParser parse) throws java.io.IOException, edu.arizona.cs.mbel.parse.MSILParseException{
      boolean tailP=false;
      ...
      if (input==0xFE){
         ...
         if (input==TailPrefixInstruction.TAIL){
            ...
            if (!(unalignedByte==1 || unalignedByte==2 || unalignedByte==4))
               throw new edu.arizona.cs.mbel.parse.MSILParseException("Instruction.readInstruction: \'unaligned.\' prefix alignment value is not one of 1,2, or 4");
      ...
      Class[] parseParams = {int.class, edu.arizona.cs.mbel.mbel.ClassParser.class};

      try{
         java.lang.reflect.Constructor newInst = clazz.getConstructor(parseParams);
         Instruction instr = (Instruction)newInst.newInstance(new Object[]{new Integer(input), parse});
